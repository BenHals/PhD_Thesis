...
from PhDCode.Classifier.advantage_fsm_o.fsm_clean import FSM_CLEAN
...
class FSMClassifier:
 ...
 similarity_measure = 'KT', merge_strategy = "sur", use_clean = False):
        ...
        if not use_clean:
            self.fsm = FSM(concept_limit =concept_limit, memory_strategy=memory_management, merge_strategy = merge_strategy)
        else:
            self.fsm = FSM_CLEAN(concept_limit =concept_limit, memory_strategy=memory_management, merge_strategy = merge_strategy)
    ...
    def _partial_fit(self, X, y, sample_weight):
        ...
        if signal_backtrack:
            ...
            self.detector = make_detector(s=self.sensitivity)
            self.warn_detector = make_detector(warn=True,s=self.sensitivity)
            self.in_warning = False
