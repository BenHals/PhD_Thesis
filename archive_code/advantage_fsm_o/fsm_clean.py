...
from PhDCode.Classifier.advantage_fsm_o.statsTests import knnTest
from PhDCode.Classifier.advantage_fsm_o.statsTests import quantTreeTest
...
class FSM_CLEAN:
    ...
    def __init__(self, concept_limit = 15, save_states = False, memory_strategy = "rA", merge_strategy = 'sur'):
    ...
    def transition_to(self, to_id, ex = 0, shadow = None):
        ...
        for s in self.states:
            self.states[s].age += 1
            self.states[s].LRU += 1
        self.get_state().last_LRU = self.get_state().LRU
        self.get_state().LRU = 0
