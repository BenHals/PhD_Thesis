...
#%%
data_dir = pathlib.Path(r"S:\PhD\Packages\PhDCode\output\expDefault\PhDCode-master-7603389")
data_dir = pathlib.Path(r"S:\PhD\Packages\PhDCode\output\sig-noise-test")
data_dir = pathlib.Path(r"S:\PhD\Packages\PhDCode\output\sig-noise-test-2")
data_dir = pathlib.Path(r"S:\PhD\Packages\PhDCode\output\sig-noise-test-3")
data_dir = pathlib.Path(r"S:\PhD\Packages\PhDCode\output\sig-noise-test-3")
data_dir = pathlib.Path(r"S:\PhD\Packages\PhDCode\output\sig-noise-3")
data_dir = pathlib.Path(r"S:\PhD\Packages\PhDCode\output\sig-noise-lock-shuffle")
data_dir = pathlib.Path(r"H:\PhD\PythonPackages\PhDCode\output\sig-noise-U-t-1")
data_dir = pathlib.Path(r"S:\PhD\Packages\PhDCode\output\sig-noise-lock-3")
data_dir = pathlib.Path(r"S:\PhD\Packages\PhDCode\output\sig-noise-lock-3\PhDCode-master-8f16e0e")
data_dir = pathlib.Path(r"S:\PhD\Packages\PhDCode\output\sig-noise-lock-4")
data_dir = pathlib.Path(r"S:\PhD\Packages\PhDCode\output\sig-noise-lock-5")
data_dir = pathlib.Path(r"S:\PhD\Packages\PhDCode\output\sig-noise-3")
data_dir = pathlib.Path(r"S:\PhD\Packages\PhDCode\output\sig-noise-3")
data_dir = pathlib.Path(r"S:\PhD\Packages\PhDCode\output\sig-noise-lock-4\PhDCode-master-5ee43bb")
data_dir = pathlib.Path(r"S:\PhD\Packages\PhDCode\output\sig-noise-6")
data_dir = pathlib.Path(r"S:\PhD\Packages\PhDCode\output\sig-noise-graph")
...
for fp, fs, sm in [('cache', 'fisher', 'metainfo'), 
    ...
    plot_feature_weights(fig, ax, fp, fs, sm)
    plt.tight_layout()
    plt.savefig(f"sig_noise_plot_l-{fp}-{fs}-{fp}.pdf")
# %%
fig, ax = plt.subplots(2, 3, sharex=True, figsize=(7.5, 5), sharey=True)
...
plt.tight_layout()
plt.savefig(f"sig_noise_plot_l-set_g-stdev.pdf")
...
# select_df = df[df['method'].isin(['cachesketchsketch_covMImetainfo100', 'cachesketchsketch_covMImetainfo2', 'cachefishermetainfo2', 'cachefisher_overallmetainfo2'])]
    ...
    plt.savefig(f"sig_noise_{test_set}_sig.pdf")
