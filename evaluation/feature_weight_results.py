...
for fp, fs, sm in [('cache', 'fisher', 'metainfo'), 
    ...
    plt.tight_layout()
    plt.savefig(f"feature_weight_plot_l-{fp}-{fs}-{fp}-opt-{using_opt}.pdf")
# %%
...
plt.tight_layout()
plt.savefig(f"feature_weight_plot_l-set-opt-{using_opt}.pdf")
# %%
...
fig.legend(handles, labels, title="Approximation\nSize", bbox_to_anchor=[1.0, 1.0], loc="upper left", frameon=False)
plt.tight_layout()
plt.savefig(f"feature_weight_plot_l-set_horizontal_2-opt-{using_opt}-stdev.pdf", bbox_inches='tight')
