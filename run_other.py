...
from PhDCode.Data.load_data import \
...
from PhDCode.Classifier.simple_CD_classifier import CDClassifier
...
from skmultiflow.meta.streaming_random_patches import StreamingRandomPatchesClassifier
from skmultiflow.meta.learn_nse import LearnPPNSEClassifier
...
def make_stream(options):
    ...
    # print(concepts)
    stream_concepts, length = get_inorder_concept_ranges(concepts, seed=options['seed'], repeats=options['repeats'], concept_max=options['concept_max'], repeat_proportion=options['repeatproportion'])
...
def process_option(option):
    ...
    def get_discrimination_results(log):
        ...
        for chunk in chunks:
            ...
            if len(all_state_active_similarity.columns) > 1:
    ...
    classifier = None
    if option['framework'] == "HoeffdingTree":
        classifier = HoeffdingTreeSHAPClassifier()
    if option['framework'] == "ARF":
        classifier = AdaptiveRandomForest()
    if option['framework'] == "OzaBag":
        classifier = OzaBagging()
    if option['framework'] == "SRP":
        classifier = StreamingRandomPatchesClassifier(n_estimators=10)
    if option['framework'] == "PPNSE":
        classifier = LearnPPNSEClassifier()
    if option['framework'] == "DWM":
        classifier = DynamicWeightedMajorityClassifier()
    if option['framework'] == "simpleCD":
        classifier = CDClassifier(learner = learner)
    if classifier is None:
        raise ValueError(f"{option['framework']} is not a valid framework option.")
    ...
    def compare_options(A, B):
        for k in A:
            if k in ['log_name', 'package_status']:
                continue
    ...
    monitoring_data = []
    monitoring_header = ('example', 'y', 'p', 'is_correct', 'right_sum', 'wrong_sum', 'overall_accuracy', 'active_model', 'ground_truth_concept', 'drift_occured', 'change_detected', 'model_evolution', 'active_state_active_similarity', 'active_state_buffered_similarity', 'all_state_buffered_similarity', 'all_state_active_similarity', 'cpu_time', 'ram_use')
    print(option)
    ...
    for i in tqdm.tqdm(range(option['length'])):
        ...
        current_active_model = classifier.active_state if hasattr(classifier, 'active_state') else -1
        ...
        observation_monitoring['change_detected'] = classifier.detected_drift if hasattr(classifier, 'detected_drift') else -1
        observation_monitoring['model_evolution'] = classifier.get_active_state().current_evolution if hasattr(classifier, 'get_active_state') else -1

        if hasattr(classifier, 'monitor_active_state_active_similarity') and classifier.monitor_active_state_active_similarity is not None:
        ...
        if hasattr(classifier, 'monitor_active_state_buffered_similarity') and classifier.monitor_active_state_buffered_similarity is not None:
        ...
        buffered_data = classifier.monitor_all_state_buffered_similarity if hasattr(classifier, 'monitor_all_state_buffered_similarity') else None
        ...
        all_state_active_similarity = classifier.monitor_all_state_active_similarity if hasattr(classifier, 'monitor_all_state_active_similarity') else None
        ...
        observation_monitoring['detected_drift'] = classifier.detected_drift if hasattr(classifier, 'detected_drift') else False
...
if __name__ == "__main__":
    ...
    my_parser.add_argument('--datasets', default='Arabic', nargs="*", type=str)
    my_parser.add_argument('--seeds', default=None, nargs="*", type=int)
    my_parser.add_argument('--seedaction', default="reuse", type=str, choices=['new', 'list', 'reuse'])
    ...
    my_parser.add_argument('--framework', default='HoeffdingTree', nargs="*", type=str)
    my_parser.add_argument('--window_size', default=150, nargs="*", type=int)
    my_parser.add_argument('--sim_gap', default=10, nargs="*", type=int)
    my_parser.add_argument('--fp_gap', default=10, nargs="*", type=int)
    ...
    my_parser.add_argument('--ifeatures', nargs="*", help="set features to be ignored, any meta-information feature")
    ...
    if given_options:
        ...
        for framework in args.framework:
            for ds_name, ds_type in dataset_options:
                ...
                for seed in seeds:
                    for sim_opt, ws_opt, sim_gap_opt, fp_gap_opt, na_fp_gap_opt, ob_gap_opt, sim_std_opt, min_sim_opt, max_sim_opt, br_opt in classifier_options:
                        option = {
