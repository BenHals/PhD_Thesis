...
class SingleTree:
    ...
    def __init__(self, tree, normalize=False, scaling=1.0, data=None, data_missing=None):
        ...
        # deprecated dictionary support (with sklearn singlular style "feature" and "value" names)
            ...
            num_nodes = tree._decision_node_cnt + tree._active_leaf_node_cnt + tree._inactive_leaf_node_cnt
            # print(num_nodes)
            ...
            def buildTree(index, node):
                ...
                # print(tree.classes)
                node_values = []
                for e in tree.classes:
                    if e not in node._observed_class_distribution:
                        node_values.append(0)
                    else:
                        node_values.append(int(node._observed_class_distribution[e]))
                if index < num_nodes:
                ...
                if node.subtree_depth() == 0:
                    ...
                    if index < num_nodes:
                        self.features[index] = node._split_test._att_idx 
                        try:#index of the feature we split on, not available for leaf, int
                            self.thresholds[index] = node._split_test._att_value #threshold for the feature, not available for leaf, float
                        except:
                            split_value = np.mean(list(node._split_test._reverse_branch_mapping.values()))
                            # try:
                            #     split_value = node._split_test._reverse_branch_mapping[split_index]
                            # except:
                            #     print(node._split_test._att_idx)
                            #     print(node._split_test._reverse_branch_mapping)
                            #     raise ValueError()
                            self.thresholds[index] = split_value #threshold for the feature, not available for leaf, float
                            # print(node._split_test.get_split_value())
                    ...
                    for i in range(2, node.num_children()):
                        idx = buildTree(idx, node.get_child(i))
