...
from PhDCode.Classifier.airstream_model.systemStats import systemStats
...
class AirstreamClassifier:
    ...
    def partial_fit(self, X, y, classes=None, sample_weight=None, masked = False):
        ...
        self.get_active_state().classes = self.classes
        self.get_active_state().classifier.classes = self.classes
        if y is not None:
    ...
    def _partial_fit(self, X, y, sample_weight, masked = False):
        ...
        if not masked:
            ...
            # Update age tracking statistics
            for state in self.state_repository.values():
                if state.id != self.active_state_id:
                    state.inactive_update()
                else:
                    state.active_update()
        ...
        if found_change:
            ...
            if can_find_good_model:
                ...
                l = self.learner()
                l.classes = self.classes
                self.place_restorepoint(ranked_alternatives, l, 'transition')
        ...
        print(len(self.state_repository))
    ...
    def rank_inactive_models_for_suitability(self):
        ...
        shadow_model.classes = self.classes
        shadow_seen = 0
        shadow_predictions = []
    ...
    def test_inactive_states(self, temporal_X, label, sample_weight, p):
        ...
        for inactive_state_id in self.testing_state_repository:
            # In some cases we may delete a state, for example due to memory management, after
            # after the testing state repository was created. In this case, we don't want to test
            # the deleted state.
            if inactive_state_id not in self.state_repository:
                continue
