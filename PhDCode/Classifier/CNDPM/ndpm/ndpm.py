...
from PhDCode.Classifier.CNDPM.ndpm.priors import CumulativePrior


class Ndpm(nn.Module):
    def __init__(self, config, writer: SummaryWriter, use_prior=True):
        super().__init__()
        self.config = config
        ...
        print(f"Use prior: {self.use_prior}")
    ...
    def forward(self, x, return_assignments=False):
        ...
        if self.use_prior:
        # TODO: Test no prior 
        # When concepts are temporally stable, it is hard to use new ones with the prior
            log_joint = log_prior + log_evid  # [B, K, 1]
        else:
            log_joint = log_evid  # [B, K, 1]
