...
from PhDCode.Classifier.advantage_fsm.fsm import FSM
from PhDCode.Classifier.advantage_fsm.fsm_clean import FSM_CLEAN
from PhDCode.Classifier.advantage_fsm.systemStats import systemStats
...
class FSMClassifier:
    ...
    sensitivity = 0.05,
 run_non_active_states = False, concept_chain = None, optimal_selection = False, optimal_drift = False, rand_weights = True, poisson = 10,
 similarity_measure = 'KT', merge_strategy = "sur", use_clean = True, merge_similarity = 0.9, drift_detector = 'adwin'):
        ...
        # print(self.concept_limit)
        # print(self.memory_management)
        # print(self.window)
        # print(self.sensitivity)
        # print(self.poisson)
        # print(self.rand_weights)
        ...
        self.fsm.observed_deletions = []
        self.fsm.observed_merges = {}
        ...
        # Monitoring, record the last calculated 'relevance' of each state.
        # The value should be the most up to date record of the similarity between
        # a state representation and current data.
        # In this system, our representation is 'error rate'
        # and similarity is the stored error rate - current error rate.
        # We calculate this only when a concept drift detector triggers.
    ...
    def partial_fit(self, X, y, classes = None, sample_weight = None):
        ...
        if self.classes is None and classes is not None:
            ...
            self.fsm.classes = classes
            self.fsm.get_state().classes = classes
        if y is not None:
    ...
    def _partial_fit(self, X, y, sample_weight):
        ...
        if signal_backtrack:
            ...
            #logging.info(f"old ID {old_state.id}")
            shadow_state = old_state.shadow_state
            #logging.info(f"shadow ID {shadow_state.id}")
            new_state_id = self.fsm.make_state(self.learner)
            #logging.info(f"new ID {new_state_id}")
            ...
            #logging.info(f"r_state id {r_state.id}")
            if r_state.id != old_state.id:
                print("PROBLEM")
                print(f"r_state id {r_state.id}")
                print(f"old_state id {old_state.id}")
                # exit()
            else:
        ...
        self.fsm.get_state().main_model.partial_fit(np.asarray([X]), np.asarray([y]), sample_weight = np.asarray([sample_weight]), classes=self.classes)
        ...
        if (not skip_selection) and (not self.optimal_drift):
            ...
            # # From the detector we can get an estimate of the current active state's relevance
            self.state_relevance[self.fsm.active_state_id] = 1 - self.detector.estimation
        ...
        if(found_change):
            ...
            #logging.info(" ")
            #logging.info(f"change detected at {self.ex}")

            if not self.concept_chain is None:
                Last_ccex = None
                for ccex in self.concept_chain:
                    if ccex > self.ex:
                        #logging.info(f"Real cc: {Last_ccex}, {ccex}")
                        if know_optimal:
                            ...
                            #logging.info(f"Real Transition: {gt_alias}")
                        break
                    Last_ccex = ccex
            ...
            if not skip_selection:
                ...
                if self.similarity_measure == 'PREVACC':
                    ...
                    new_state_id, is_recurring,shadow, state_relevance = self.fsm.get_KT_reccurence(recent_window, self.learner, self.system_stats)
                    self.state_relevance.update(state_relevance)
                else:
            ...
            # Logging
            # if not self.suppress: #logging.info(f"Transitioned to {new_state_id}")
            #logging.info(f"Transitioned to {new_state_id}")
            # if know_optimal:
            #logging.info(f"Real Transition: {gt_alias}, correct {new_state_id == gt_alias}")
        ...
        if not (hasattr(self.fsm.get_state().main_model, 'splits_since_reset')):
            raise ValueError(f"Base classifier needs a 'splits_since_reset' property to track evolutions. Please add. It should increment by 1 when classifier behaviour changes significantly, e.g., a decision tree growing a new branch")
    ...
    def reset_stats(self, rem_state_log=False):
        """
