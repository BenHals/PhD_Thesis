...
def HoeffdingTreeSHAP(max_byte_size=33554432, memory_estimate_period=1000000, grace_period=200, split_criterion='info_gain',
    ...
    warnings.warn("'HoeffdingTreeSHAP' has been renamed to 'HoeffdingTreeSHAPClassifier' in v0.5.0.\n"
                  "The old name will be removed in v0.7.0", category=FutureWarning)
    return HoeffdingTreeSHAPClassifier(max_byte_size=max_byte_size,
...
class HoeffdingTreeSHAPClassifier(BaseSKMObject, ClassifierMixin):
    ...
    >>> from skmultiflow.trees import HoeffdingTreeSHAPClassifier
    >>>
    >>> # Setting up a data stream
    >>> stream = SEAGenerator(random_state=1)
    >>>
    >>> # Setup Hoeffding Tree estimator
    >>> ht = HoeffdingTreeSHAPClassifier()
    >>>
    ...
    def __init__(self,
        ...
        """ HoeffdingTreeSHAPClassifier class constructor."""
        super().__init__()
        ...
        self.evolution = 0
        self.splits_since_reset = 0

    def make_shap_model(self):
        return shap.TreeExplainer(self, feature_perturbation="tree_path_dependent", check_additivity=False)
    ...
    def partial_fit(self, X, y, classes=None, sample_weight=None):
        ...
        * If growth is allowed and the number of instances that the leaf has
        ...
        if self.shap_model is None:
            self.shap_model = self.make_shap_model()
        return self
    ...
    def _partial_fit(self, X, y, sample_weight):
        ...
        if isinstance(leaf_node, LearningNode):
            ...
            # Can cause underflow error, but this doesn't matter
            np.seterr(under='ignore')
            # np.seterrcall(lambda x, f: logging.info(str(f) + str(f)))
        ...
        # (in a multiway test)
            ...
            old_shap = self.shap_model
            try:
                self.shap_model = self.make_shap_model()
            except Exception as e:
                print(e)
                self.shap_model = old_shap
            self.evolution += 1
            self.splits_since_reset += 1
    ...
    def predict_proba(self, X):
        ...
        predictions = []
        for i in range(r):
            # NB can cause underflow error, but this doesn't matter
            np.seterr(under='ignore')
            # np.seterrcall(lambda x, f: logging.info(str(f) + str(f)))
    ...
    def estimate_model_byte_size(self):
        ...
        # actual_model_size_orig = calculate_object_size(self._tree_root)
        # actual_model_size = actualsize(self._tree_root)
        # print(actual_model_size_orig, actual_model_size, actual_model_size/actual_model_size_orig)
        actual_model_size = calculate_object_size(self._tree_root)
