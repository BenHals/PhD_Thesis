...
import sys
import math
from PhDCode.Classifier.advantage_fsm.modelStats import modelStats
import numpy as np
sys.path.append('../similarityMeasure')
from PhDCode.Classifier.advantage_fsm.statsTests import knnTest
from PhDCode.Classifier.advantage_fsm.statsTests import quantTreeTest
...
class FSM_CLEAN:
    ...
    def construct_state(self, state_id, learner):
        return State(state_id, learner, self.classes)
    ...
    def get_KT_reccurence(self, recent_window, learner, system_stats):
        ...
        for s_i, state in enumerate(self.states.values()):
            ...
            # state_relevance[state.id] = 1 - abs((k_temporal_acc * active_state_penalty) - (recent_temp * t))
            state_relevance[state.id] = 1 - abs((accuracy * active_state_penalty) - (recent_acc * t))
        ...
        return (new_state_id, is_recurring, deepcopy(shadow_state.main_model), state_relevance)
    ...
    def cull_states(self, recent_window):
        ...
        if len(self.states.values()) > self.concept_limit and self.concept_limit > 0:
            while len(self.states.values()) > self.concept_limit:
                ...
                self.observed_deletions.append(cull_state)
