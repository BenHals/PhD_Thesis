...
from PhDCode.Classifier.feature_selection.fisher_score import fisher_score
...
from PhDCode.Classifier.fingerprint import (FingerprintBinningCache,
                                           FingerprintCache,
                                           FingerprintSketchCache,
                                           MinimalFingerprint)
from PhDCode.Classifier.metafeature_extraction import (
...
from PhDCode.Classifier.memory_management.memory_management import repository_memory_management
...
class SELeCTClassifier:
    def __init__(self,
        ...
        init_state = ConceptState(init_id, self.learner(), self.fingerprint_update_gap,
                                  ...
                                  window_size=window_size, min_window_size = self.min_window_size, dirty_data_length=self.window_size, normalizer=self.normalizer, fingerprint_grace_period=self.fingerprint_grace_period, estimator_risk_level=self.state_estimator_risk, buffer_ratio=self.buffer_ratio, metainfo_method=self.MI_calc)
    ...
    def construct_state_object(self, id=-1):
        return ConceptState(id, self.learner(), self.fingerprint_update_gap,
                                  fingerprint_method=self.fingerprint_method, fingerprint_bins=self.fingerprint_bins,
                                  window_size=self.window_size, min_window_size = self.min_window_size, dirty_data_length=self.window_size, normalizer=self.normalizer, fingerprint_grace_period=self.fingerprint_grace_period, estimator_risk_level=self.state_estimator_risk, buffer_ratio=self.buffer_ratio, metainfo_method=self.MI_calc)
    ...
    def _accuracy_similarity(self, current_metainfo, state=None, fingerprint_to_compare=None):
        ...
        if fingerprint_to_compare is None:
            ...
            fingerprint_accuracy = fingerprint_to_compare.flat_ignore_vec[0]
    ...
    def record_state_stable_similarity(self, state):
        ...
        metainfo, metainfo_flat = state.get_stable_metainfo(method=self.MI_calc, add_to_normalizer=valid_window)
    ...
    def init_normalizer(self):
        ...
        if self.normalizer.seen_stats < self.normalizer_grace_period and self.get_active_state().has_valid_head_window(self.min_window_size):
            self.get_active_state().get_head_metainfo(method=self.MI_calc, add_to_normalizer=True)
    ...
    def evaluate_all_fingerprint_similarity(self, metainfo, metainfo_flat, use_stable, add_to_normalizer, update_na_record, record_likelihood):
        ...
        for concept_id in [*self.state_repository.keys(), -1]:
            ...
            if concept_id != self.active_state_id:
                ...
                concept_metainfo, concept_metainfo_flat = window_metainfo_func(method=self.MI_calc, add_to_normalizer=add_to_normalizer, base_window = active_state_window, feature_base=fingerprint_feature_base,feature_base_flat=metainfo_flat,
    ...
    def update_all_fingerprints_stable(self):
        ...
        """
        active_stable_metainfo, active_stable_metainfo_flat = self.get_active_state().get_stable_metainfo(method=self.MI_calc, add_to_normalizer=True)
    ...
    def monitor_all_fingerprints_head(self):
        ...
        """
        active_head_metainfo, active_head_metainfo_flat = self.get_active_state().get_head_metainfo(method=self.MI_calc, add_to_normalizer=True)
    ...
    def parse_apply_transition_from_alternatives(self, ranked_alternatives, use_shadow, shadow_model, can_find_good_model):
        ...
        repository_memory_management(self.state_repository, self.active_state_id, self.repository_max, self.valuation_policy, [])
    ...
    def _partial_fit(self, X, y, sample_weight, masked=False):
        ...
        if take_measurement:
            ...
            if is_valid_fingerprint(self.background_state.fingerprint):
                background_head_metainfo, background_head_metainfo_flat = self.background_state.get_head_metainfo(method=self.MI_calc, add_to_normalizer=True)
